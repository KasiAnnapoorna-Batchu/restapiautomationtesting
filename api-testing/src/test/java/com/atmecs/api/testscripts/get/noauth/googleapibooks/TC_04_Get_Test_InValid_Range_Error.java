...
/**
 * 
 * @author Kasi.Batchu This class is written to test whether invalid range error
 *         using get operation.
 *
 */
public class TC_04_Get_Test_InValid_Range_Error {
	 ...
	 * @throws Exception
	 *             Get and validate whether invalid range error when the range is
	 *             given above the max range.
	 * 
	 */
	...
	public void GetInvalidRangeError() throws FileNotFoundException, Exception {
		try {
			...
			if (response != null) {
				...
				Assert.assertTrue(
						errorMessage.contains("Values must be within the range: [0, 40]")
								&& paramName.equals("maxResults"),
